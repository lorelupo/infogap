...
def test_fact_intersection_query(src_context, tgt_context, src_lang, person_name):
    ...
    # ]
    # src_context = ['Hummus is often served as a warm dish.', 'Hummus is often served with bread for breakfast, lunch, or dinner.', 'All of the ingredients in hummus are easily found in gardens, farms, and markets.']
    # person_name = "Hummus"
...
if __name__ == "__main__":
    ...
    tgt_context_tgt = pd[pd['src_lang']==TARGETED_LANG]['tgt_context'].tolist()
    response_content_en = []
    response_content_tgt = []
    for row in pd.iterrows():
        if row['language']=='en':
            src_context = row['src_context'].tolist()
            tgt_context = row['tgt_context'].tolist()
            person_name = row['person_name']
            input_prompt, response_content, total_tokens = test_fact_intersection_query(src_context, tgt_context, 'en', person_name)
            response_content_en.append(response_content)
            
        else:
            src_context = row['src_context'].tolist()
            tgt_context = row['tgt_context'].tolist()
            person_name = row['person_name']
            input_prompt, response_content, total_tokens = test_fact_intersection_query(src_context, tgt_context, TARGETED_LANG, person_name)
            response_content_tgt.append(response_content)
    print(response_content_en)
    print(response_content_tgt)